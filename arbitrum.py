...
DEFAULT_STATE_FILE = "arbitrum_state.json"
ARBITRUM_NATIVE_SYMBOL = "ETH"
...
ARBITRUM_USDC_E_CONTRACT = "0xff970a61a04b1ca14834a43f5de4533ebddb5cc8"
SUPPORTED_ARBITRUM_TOKENS = {
    ARBITRUM_USDT_CONTRACT: "USDT-ARBITRUM",
    ARBITRUM_USDC_CONTRACT: "USDC-ARBITRUM",
    ARBITRUM_USDC_E_CONTRACT: "USDC.E-ARBITRUM",
}
    ...
    def call(self, method: str, params: list[Any]) -> Any:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                logging.warning(
                    "arbitrum rpc call %s failed, retrying (%s/%s): %s",
                    method,
                    attempt + 1,
                    self.retries,
                    exc,
...
class ArbitrumMonitor:
    def __init__(self, config: MonitorConfig) -> None:
        ...
        self.token_contracts = set(SUPPORTED_ARBITRUM_TOKENS)
            ...
            if from_monitored:
                notifications.append(
                    TransferNotification(
                        direction="出账",
                        token_symbol=ARBITRUM_NATIVE_SYMBOL,
                        amount=amount,
    ...
    def _get_token_label(self, contract_address: str, fallback: str) -> str:
        normalized = normalize_address(contract_address)
        return SUPPORTED_ARBITRUM_TOKENS.get(normalized, fallback)
...
def load_config() -> MonitorConfig:
    ...
    rpc_url = (
        ...
        or os.getenv("ARBITRUM_RPC")
        or ""
    ).strip()
    monitor_addresses_raw = (os.getenv("ARBITRUM_MONITOR_ADDRESSES") or "").strip()
    ...
    if not rpc_url:
        missing_fields.append("Arbitrum_RPC")
    if not monitor_addresses_raw:
        missing_fields.append("ARBITRUM_MONITOR_ADDRESSES")
    ...
    return MonitorConfig(
        ...
        rpc_timeout=float(os.getenv("ARBITRUM_RPC_TIMEOUT", "15")),
        rpc_retries=int(os.getenv("ARBITRUM_RPC_RETRIES", "2")),
        rpc_retry_delay=float(os.getenv("ARBITRUM_RPC_RETRY_DELAY", "1")),
        poll_interval=float(os.getenv("ARBITRUM_POLL_INTERVAL", "3")),
        confirmations=int(os.getenv("ARBITRUM_CONFIRMATIONS", "1")),
        start_block=os.getenv("ARBITRUM_START_BLOCK", "latest"),
        state_file=Path(os.getenv("ARBITRUM_STATE_FILE", DEFAULT_STATE_FILE)),
        notify_native=parse_bool(os.getenv("ARBITRUM_NOTIFY_NATIVE"), default=True),
    )
...
def main() -> None:
    ...
    config = load_config()
    monitor = ArbitrumMonitor(config)
    monitor.run()
