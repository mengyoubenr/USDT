...
DEFAULT_STATE_FILE = "polygon_state.json"
POLYGON_NATIVE_SYMBOL = "POL"
...
POLYGON_USDC_E_CONTRACT = "0x2791bca1f2de4661ed88a30c99a7a9449aa84174"
SUPPORTED_POLYGON_TOKENS = {
    POLYGON_USDT_CONTRACT: "USDT-POLYGON",
    POLYGON_USDC_CONTRACT: "USDC-POLYGON",
    POLYGON_USDC_E_CONTRACT: "USDC.E-POLYGON",
}
    ...
    def call(self, method: str, params: list[Any]) -> Any:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                logging.warning(
                    "polygon rpc call %s failed, retrying (%s/%s): %s",
                    method,
                    attempt + 1,
                    self.retries,
                    exc,
...
class PolygonMonitor:
    def __init__(self, config: MonitorConfig) -> None:
        ...
        self.token_contracts = set(SUPPORTED_POLYGON_TOKENS)
            ...
            if from_monitored:
                notifications.append(
                    TransferNotification(
                        direction="出账",
                        token_symbol=POLYGON_NATIVE_SYMBOL,
                        amount=amount,
    ...
    def _get_token_label(self, contract_address: str, fallback: str) -> str:
        normalized = normalize_address(contract_address)
        return SUPPORTED_POLYGON_TOKENS.get(normalized, fallback)
...
def load_config() -> MonitorConfig:
    ...
    monitor_addresses_raw = (os.getenv("POLYGON_MONITOR_ADDRESSES") or "").strip()
    ...
    if not rpc_url:
        missing_fields.append("Polygon_RPC")
    if not monitor_addresses_raw:
        missing_fields.append("POLYGON_MONITOR_ADDRESSES")
    ...
    return MonitorConfig(
        ...
        rpc_timeout=float(os.getenv("POLYGON_RPC_TIMEOUT", "15")),
        rpc_retries=int(os.getenv("POLYGON_RPC_RETRIES", "2")),
        rpc_retry_delay=float(os.getenv("POLYGON_RPC_RETRY_DELAY", "1")),
        poll_interval=float(os.getenv("POLYGON_POLL_INTERVAL", "3")),
        confirmations=int(os.getenv("POLYGON_CONFIRMATIONS", "1")),
        start_block=os.getenv("POLYGON_START_BLOCK", "latest"),
        state_file=Path(os.getenv("POLYGON_STATE_FILE", DEFAULT_STATE_FILE)),
        notify_native=parse_bool(os.getenv("POLYGON_NOTIFY_NATIVE"), default=True),
    )
